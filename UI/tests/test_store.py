...
class BaseTestStoreMethods:
    ...
    class TestStoreMethods(unittest.TestCase):
        ...
        def test_upload_internal_only_name(self):
            test_path = pathlib.PurePath('/sample')
            with open(self.project_dir / 'tests' / 'samples' / 'sample1', 'rb') as testfile:
                test_data = testfile.read()
            # '.unidrive_chunk' is suffix for internal chunk file
            with self.assertRaises(InvalidEntryNameError):
                self.store.upload_file(test_path.with_suffix('.unidrive_chunk'), test_data)

            # '.unidrive_recipe' is suffix for internal chunk file
            with self.assertRaises(InvalidEntryNameError):
                self.store.upload_file(test_path.with_suffix('.unidrive_recipe'), test_data)

        def test_download_internal_only_name(self):
            test_path = pathlib.PurePath('/sample')
            # '.unidrive_chunk' is suffix for internal chunk file
            with self.assertRaises(InvalidEntryNameError):
                ret = self.store.download_file(test_path.with_suffix('.unidrive_chunk'))

            # '.unidrive_recipe' is suffix for internal chunk file
            with self.assertRaises(InvalidEntryNameError):
                self.store.download_file(test_path.with_suffix('.unidrive_recipe'))

        def test_get_list_hide_internal_files(self):
            test_samples_dir = pathlib.PurePath('/samples')
            sample_files = ['sample1', 'sample2', 'sample3']
            self.store.make_dir(test_samples_dir.parent, 'samples')
            for sample in sample_files:
            ...
            names = []
            for entry in entries:
                self.assertFalse(entry.is_chunk)
                self.assertFalse(entry.is_recipe)
                names.append(entry.name)
            self.assertCountEqual(sample_files, names)
