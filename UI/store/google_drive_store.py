...
class GoogleDriveStore(Store):
    ...
    def download_file(self, path: PurePath) -> bytes:
        # prevent download'.unidrive_chunk' and '.unidrive_recipe' directly
        if path.suffix:
            if path.suffix == '.unidrive_chunk' or path.suffix == '.unidrive_recipe':
                raise InvalidEntryNameError('Cannot download .unidrive_chunk or .unidrive_recipe directly')
    ...
    def upload_file(self, path: PurePath, data: bytes) -> bool:
        # prevent upload '.unidrive_chunk' and '.unidrive_recipe'
        if path.suffix:
            if path.suffix == '.unidrive_chunk' or path.suffix == '.unidrive_recipe':
                raise InvalidEntryNameError('Cannot use .unidrive_chunk or .unidrive_recipe')
    ...
    def get_list(self, path: PurePath) -> List[DirectoryEntry]:
        ...
        filtered_results = [x for x in results if not x.is_recipe or not x.is_chunk]
        return results
